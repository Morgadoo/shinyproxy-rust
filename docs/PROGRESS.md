...
| P10 | Operational features (logs, metrics, timeouts, stats) | ✅ | release timers (heartbeat timeout, max lifetime, logout), management server (health/readiness/recyclable/prometheus on `management.server.port`), Micrometer-compatible metrics, container log collection, `proxy.log-as-json` + `logging.*`, CSV and SQL usage statistics collectors with SpEL attribute expressions. Documented gaps: S3 log storage, InfluxDB collector, `logging.requestdump` |
