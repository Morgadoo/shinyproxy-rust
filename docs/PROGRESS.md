...
| `shinyproxy` Redis store (end to end, `SP_TEST_REDIS=1`) | 6 | two servers sharing apps (start on one, see and stop on the other), shared host port allocation from one range, shared heartbeats, the leader election, shared sessions (login on one server, use the app API on the other, the namespace of Spring Session, the user counts, sign out everywhere), a rolling update where the newest `proxy.version` takes the leadership |
