...
| `shinyproxy` OpenID Connect (end to end) | 5 | the whole flow against a fake provider with real RS256 id tokens (redirects, code exchange, verification, user info, groups, access token in the app), a wrong state, roles claims of every shape, PKCE |
