...
| P10 | Operational features (logs, metrics, timeouts, stats) | ✅ | release timers (heartbeat timeout, max lifetime, logout), management server (health/readiness/recyclable/prometheus on `management.server.port`), Micrometer-compatible metrics, container log collection, `proxy.log-as-json` + `logging.*`, CSV, SQL and InfluxDB usage statistics collectors with SpEL attribute expressions. Documented gaps: S3 log storage, `logging.requestdump` |
...
The features inside these files that are deliberately not implemented (S3 log storage,
