...
| `shinyproxy` Redis store (end to end, `SP_TEST_REDIS=1`) | 3 | two servers sharing apps (start on one, see and stop on the other), shared host port allocation from one range, shared heartbeats, realm isolation |
