...
| Pre-initialized containers | `minimum-seats-available`, `seats-per-container`, `allow-container-re-use`, `scale-down-delay` and `proxy.seat-wait-time`: the scaler keeps containers running (only on the leader), users claim a seat and are served under the public path of their container (`/api/route/{id}/`), released seats are re-used or the container is marked for removal, `max-total-instances` caps the seats, and `DELETE /admin/delegate-proxy[?id=&specId=]` marks containers for removal. `seats_unclaimed`, `seats_claimed` and `seats_creating` are exposed as metrics | ✅ (the seats live in memory, so they are not shared between the servers of a realm — a Redis seat store is not implemented) |
