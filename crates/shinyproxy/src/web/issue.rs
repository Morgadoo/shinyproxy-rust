...
pub fn build_report(
    ...
    if let Some(proxy) = proxy {
        ...
        // container logs are attached in P10, when log collection lands; the paths are mentioned as the
        // Java implementation does when the files do not exist
        if let Some(log_path) = &state.settings.proxy.container_log_path {
            body.push_str(&format!(
                "Log (stdout): {log_path}/{}_stdout.log\n",
                proxy.id
            ));
            body.push_str(&format!(
                "Log (stderr): {log_path}/{}_stderr.log\n",
                proxy.id
            ));
        }
    }
    ...
    fn builds_the_java_mail_body() {
        ...
        assert_eq!(
            ...
            "This is an error report generated by ShinyProxy\n\
             ...
             Log (stdout): /var/log/sp/proxy-1_stdout.log\n\
             Log (stderr): /var/log/sp/proxy-1_stderr.log\n"
        );
    }
